...
function HygieneNumberIcon({ number }: HygieneNumberIconProps) {
  ...
  return (
    <div className="flex h-[65px] w-[60px] shrink-0 items-start justify-center text-[#004374]">
      <svg
        role="img"
      ...
      >
        <circle cx="255" cy="255" r="243.5" fill="none" stroke="currentColor" strokeWidth="23" />
...
function HygieneBenefitCard({ number, title, description, index }: HygieneBenefitCardProps) {
  return (
    <Card
      ...
      className={cn(
        'group flex flex-col items-center gap-0 rounded-[20px] border border-[#DBDBDB] bg-white p-5 text-center shadow-[0_0_10px_2px_rgba(0,0,0,0.10)] transition duration-300 hover:shadow-[0_0_15px_10px_rgba(0,67,116,0.24)] md:h-[277.5px] md:min-h-0 tablet-plus:h-full wide:h-[304px]',
        index >= 4 && 'md:h-[257px] tablet-plus:h-full wide:h-[304px]'
      )}
    >
      <HygieneNumberIcon number={number} />
      <h3
        className={cn(
          'mt-[18px] mb-0 w-full font-sans text-[18px] font-bold leading-[21.6px] tracking-[-0.6px] text-[#004374] md:text-[22px] md:leading-[26.4px] md:tracking-[-1px] lg:tracking-normal',
          index === 7 && 'md:tracking-[-1.1px] lg:tracking-normal'
        )}
      >
        {title}
      </h3>
...
export function HygieneBenefitsSection() {
  ...
  return (
    <section
      ...
      className="bg-white px-[30px] pt-[30px] pb-[33px] text-center [contain-intrinsic-size:auto_2435px] [content-visibility:auto] md:px-10 md:pt-10 md:pb-[43.5px] md:[contain-intrinsic-size:auto_1412px] lg:px-20 lg:pt-20 lg:pb-[83.5px] lg:[contain-intrinsic-size:auto_892px]"
    >
      <div className="mx-auto flex max-w-[1140px] flex-col items-center">
        <EntranceReveal
          ...
          className="w-full font-serif text-[25px] font-bold leading-[25px] tracking-normal text-[#4F4F4F] md:w-[85%] md:text-[35px] md:leading-[35px] lg:w-1/2"
        >
        ...
        <EntranceReveal
          as="p"
          effect="fadeInUp"
          className="mt-5 w-full font-sans text-[15px] font-normal leading-[22.5px] tracking-[-0.5px] text-[#545454] md:tracking-normal"
        >
          {hygieneBenefits.subtitle}
        </EntranceReveal>

        <div className="mt-[35px] grid w-full grid-cols-1 items-stretch gap-x-5 gap-y-[30px] md:grid-cols-2 lg:grid-cols-4">
        ...
        <EntranceReveal
          ...
          effect="zoomIn"
          slow={false}
          data-dr-helio-hygiene-cta
          href={whatsappUrl}
          className="relative -top-px mt-10 inline-flex h-[60px] w-full items-center justify-center rounded-[0_20px_0_20px] bg-[linear-gradient(360deg,#0066AF_0%,#004374_100%)] px-[15px] py-[15px] font-sans text-[15px] font-bold leading-[15px] text-white uppercase shadow-[0_0_23px_-2px_rgba(0,81,140,0.8)] transition hover:-translate-y-0.5 hover:brightness-110 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#0066af] md:mt-[60px] md:h-[45px] md:w-[392px] md:px-10 lg:h-[46px] lg:w-[412px] lg:text-[16px] lg:leading-[16px]"
        >
          <span className="inline-flex max-w-full flex-row-reverse items-center justify-center gap-[5px] md:max-lg:relative md:max-lg:-top-px">
            <span className="flex size-[15px] shrink-0 items-center justify-center lg:size-4">
              <DrHelioIcon name="whatsapp" className="size-[15px] shrink-0 lg:size-4" />
            </span>
            <span className="min-w-0 text-center tracking-[-0.5px] whitespace-normal md:tracking-[-0.47px] lg:tracking-[-0.5px] lg:whitespace-nowrap">
              {cta}
            </span>
          </span>
        </EntranceReveal>
